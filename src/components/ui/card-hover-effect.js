...
export const HoverEffect = ({
  ...
  return (
    <div
      className={cn("grid grid-cols-1 md:grid-cols-2  lg:grid-cols-3  py-10 hover:cursor-default", className)}>
      {items.map((item, idx) => (
        <a
          href={item?.link}
          key={`${item.title}-${idx}`}
          ...
          <AnimatePresence>
            {hoveredIndex === idx && (
              <motion.span
                className="absolute inset-0 h-full w-full bg-neutral-200 dark:bg-blue-200 block  rounded-3xl"
